...
def plot_liquidity_map(holders_data, pool_name, protocol, version, chain):
    ...
    treemap_fig = go.Figure(go.Treemap(
        labels=labels,
        parents=parents,
        values=values,  # Proportional to the size of the rectangle
        text=texts,
        textposition='middle center',
        #textfont=dict(size=[value * 0.5 for value in values]),
        marker=dict(
            colorscale='Blues',
            line=dict(width=1, color='white')
        ),
    ...
    treemap_fig.update_layout(
        title={
            ...
            'xanchor': 'center',
            'yanchor': 'top',
            'font': dict(size=24, weight='bold')  # Increase the font size of the title
        },
        margin=dict(t=50, l=25, r=25, b=25),
        height=600  # Increase the height of the treemap for better visibility
    )

    # Display the treemap
    st.plotly_chart(treemap_fig, use_container_width=True)

    # Create the table with the ranking data
    ranking_data = [
        [f"#{i+1}", holder['address'], f"{holder['percentage']:.2f}%", holder['type']]
        for i, holder in enumerate(holders_data)
    ]
    ranking_headers = ["Rank", "Wallet Address", "Percentage", "Type"]
    ...
    table_fig = go.Figure(data=[go.Table(
        ...
        cells=dict(values=[col for col in zip(*ranking_data)],
                   line_color='darkslategray',
                   fill_color='lavender',
                   align='left',
    ...
    table_fig.update_layout(
        ...
        width=1200  # Increase the width of the table
    )
